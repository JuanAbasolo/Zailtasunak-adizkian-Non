...
Aukera-arrazoia erabiltzeko joera dago 2x2 dimentsioetako alderaketak egiteko, guk, lan honetarako horren erabilera baino $\chi^2$rena lehenesten dugu arrazoi bigatik. Batetik, aukera-arrazoien indargunea banaketaren aurretiko ezagutzan oinarritzen delako eta corpuseko datuak ausaz hala hartuak izan direlako. Beste alderdi batetik, aurrez egindako alderaketekin koherentzia gordetzeko, 2x2 taulok beste batzuen erredukzioa direlako.
...
```{r baietzNGvLZ0, fig.cap="Fonazio luzatuak eta aditz nagusien banaketa baiezko esaldietan"}
...
Irudian ikusten da, aditz nagusian fonazioa luzatzeko joera handia dela aditz nagusietan baiezko esaldietan. Izan ere, konfigurazio horretan etiketatutako fonazio luzatuen herenak aditz nagusian neurtu dira. Kolore gorri argiak adierazten du fonazio luzatuak aukera baxuagoa duela agertzeko aditz nagusia ez diren elementuetan. Hor irudikatuta dagoen banaketak $\chi^2$<sub>(`r chtx1$parameter`, N=`r sum(chtx1$observed)`)</sub> = `r round(chtx1$statistic,3)`, p = `r round(chtx1$p.value,5)` balioa du.

```{r ezetzNGvLZ0, fig.cap="Fonazio luzatuak eta aditz nagusien banaketa ezezko esaldietan"}
...
Ezeko esaldietan, berriz, ez da antzematen ezelako joerarik irudian eta alderik balego ere, adierazgarritasunik ez da ikusi $\chi^2$ banaketarekiko aldean honako balioa hartuta:
 $\chi^2$<sub>(`r chty1$parameter`, N=`r sum(chty1$observed)`)</sub> = `r round(chty1$statistic,3)`, p = `r round(chty1$p.value,5)`


### Zalantza adierazleen banaketak adizki laguntzaile eta trinkoen kasuetan



```{r laguntzaile, fig.cap="Fonazio luzatua aurrekoan eta aditz trinkoak"}
...
$\chi^2$<sub>(`r chttrinluz1$parameter`, N=`r sum(chttrinluz1$observed)`)</sub> = `r round(chttrinluz1$statistic,3)`, p = `r round(chttrinluz1$p.value,5)`

```{r trinko, fig.cap="Fonazio luzatua aurrekoan eta aditz laguntzailea"}
...
$\chi^2$<sub>(`r chtlagluz1$parameter`, N=`r sum(chtlagluz1$observed)`)</sub> = `r round(chtlagluz1$statistic,3)`, p = `r round(chtlagluz1$p.value,5)`


### Aditz nagusiaren aspektua eta fonazio luzatua

```{r adnagaspek, fig.cap="Fonazio luzatua aditz nagusietan, aspektuen arabera"}
tadnagaspluz0 <- table(EUDIA7$AD.NG.Q.ASP,EUDIA7$ZA.LUZ0)[-3,]
dimnames(tadnagaspluz0) <- list(Aspektua = c("gabekoa", "burutua", "ez burutua", "geroa"),
...
$\chi^2$<sub>(`r chtadnagaspluz0$parameter`, N=`r sum(chtadnagaspluz0$observed)`)</sub> = `r round(chtadnagaspluz0$statistic,3)`, p = `r round(chtadnagaspluz0$p.value,5)`
